  ...
  <BCard no-body>
    <BCardHeader class="border-light">
      <h5 class="card-title mb-0 d-flex align-items-center gap-2">
        <Icon icon="briefcase" class="text-primary" />
        Envoi pro
      </h5>
    </BCardHeader>
    <BCardBody>
      <BForm @submit.prevent="handleSend">
        ...
        <BRow v-if="sendMode === 'custom'" class="mb-3">
          <BCol md="12">
            <div class="mb-3">
              <label class="form-label">Pro(s) spécifique(s) <span class="text-danger">*</span></label>
              <Multiselect
                v-model="selectedRecipients"
                :options="recipients"
                track-by="value"
                label="label"
                placeholder="Sélectionner un ou plusieurs pros..."
                :searchable="true"
                :allow-empty="false"
                :multiple="true"
                select-label=""
          ...
          <BCol md="12">
            <div class="mb-3">
              ...
              <BFormTextarea
                ...
                rows="3"
                placeholder="Contenu de la notification pro..."
                required
              />
            </div>
          </BCol>
...
import { sendProNotification } from '~/services/push-notifications-pro.service'
...
const sendModeOptions = [
  { text: 'Tous les pros', value: 'broadcast' },
  { text: 'Pros spécifiques', value: 'custom' },
]
...
const handleSend = async () => {
  if (sendMode.value === 'custom' && !selectedRecipients.value.length) {
    emit('error', 'Veuillez sélectionner au moins un pro.')
    return
  }

  sending.value = true
  try {
    await sendProNotification({
    ...
    resetForm()
    emit('sent', 'Notification pro envoyée avec succès !')
  } catch (e) {
    emit(
      'error',
      e instanceof Error && e.message ? e.message : "Erreur lors de l'envoi de la notification pro.",
    )
    console.error(e)
  } finally {
    sending.value = false
